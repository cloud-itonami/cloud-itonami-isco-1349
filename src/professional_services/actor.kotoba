(ns professional-services.actor
  "ProfessionalServicesActor — the ISCO-08 1349 independent
  professional-services-management actor as a `langgraph.graph/state-
  graph` (per ADR-2607011000 / CLAUDE.md Actors section). One graph
  run = one services operation request (intake → advise → govern →
  ...
  The unconditional invariant: the ServicesManagementAdvisor can never
  directly commit a record or dispatch a robot action the
  ProfessionalServicesGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [professional-services.advisor :as advisor]
            [professional-services.governor :as governor]
            [professional-services.store :as store]))

(defn build-graph
  "Build a compiled ProfessionalServicesActor graph. `store` implements
  `professional-services.store/Store`. `advisor` implements
  `professional-services.advisor/Advisor` (defaults to
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:engagement-id (:engagement-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
